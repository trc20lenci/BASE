...
на дорожке, громкость реально регулируется. Чего пока нет: синхронного
проигрывания дорожки вместе с видео в живом предпросмотре и её
микширования в финальный экспортируемый файл — это происходит на этапе
рендера в `VideoExportEngine`, который сам ещё не реализован (см. ниже).
